...
function Harness({ initial = "en" as CvLanguage }: { initial?: CvLanguage }) {
...
describe("LanguageCombobox", () => {
  it("exposes combobox semantics and selects with pointer", async () => {
    ...
    await user.click(within(listbox).getByRole("option", { name: /Portugués.*Português/ }));
    expect(input).toHaveAttribute("aria-expanded", "false");
  ...
  it("filters while typing and selects with the keyboard", async () => {
    ...
    expect(options[0]).toHaveTextContent("Neerlandés");
