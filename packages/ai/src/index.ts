...
export class PortableCvAiService implements CvAiService {
  ...
  async rewrite(cv: CvData, targetRole: string, jobDescription: string | undefined, language: Language): Promise<CvData> {
    ...
    const prompt = language === "es"
      ? `Mejora este CV manteniendo exactamente el esquema. ${languageRule} Redaccion profesional, bullets claros y medibles cuando sea posible, resumen breve orientado al rol. No cambies datos factuales. Devuelve solo JSON.\n\nRol objetivo:\n${targetRole}${jobContext}\n\nJSON actual:\n${JSON.stringify(cv)}`
      : `Improve this CV while keeping exactly the schema. ${languageRule} Use professional wording, clear measurable bullets where possible, and a concise role-oriented summary. Do not change factual data. Return JSON only.\n\nTarget role:\n${targetRole}${jobContext}\n\nCurrent JSON:\n${JSON.stringify(cv)}`;
    return parseCvJson(await this.complete(prompt));
  }
  ...
  async modify(cv: CvData, instruction: string, jobDescription: string | undefined, language: Language): Promise<CvData> {
    ...
    const jobContext = createJobContext(jobDescription, language);
    const prompt = language === "es"
      ? `Modifica este CV siguiendo la instruccion del usuario y manteniendo exactamente el esquema. ${languageRule} Puedes mejorar redaccion, agregar habilidades si el usuario las entrega, reorganizar secciones o adaptar al rol. No inventes datos factuales. Devuelve solo JSON.\n\nInstruccion:\n${instruction}${jobContext}\n\nJSON actual:\n${JSON.stringify(cv)}`
      : `Modify this CV following the user's instruction while keeping exactly the schema. ${languageRule} You may improve wording, add skills if supplied, reorganize sections, or tailor it to a role. Do not invent factual data. Return JSON only.\n\nInstruction:\n${instruction}${jobContext}\n\nCurrent JSON:\n${JSON.stringify(cv)}`;
    return parseCvJson(await this.complete(prompt));
  }

  async translate(cv: CvData, language: Language): Promise<CvData> {
    const target = language === "es" ? "Spanish" : "English";
    const prompt = `Translate every user-visible text value in this CV JSON into ${target} while keeping exactly the same schema, arrays, order, and factual meaning. Do not translate proper names, company names, locations, email addresses, phone numbers, URLs, dates, or technical terms that should remain unchanged. Translate titles, summary, experience bullets, education details, section group names, language levels, and skill descriptions where appropriate. Return JSON only.\n\nJSON:\n${JSON.stringify(cv)}`;
    return parseCvJson(await this.complete(prompt));
  }
...
function importPrompt(description: string, language: Language): string {
  return language === "es"
...
function repairPrompt(description: string, cv: CvData, language: Language): string {
  return language === "es"
...
function normalizeCv(cv: CvData, sourceText: string): CvData {
...
function hasSuspiciousExperienceDistribution(cv: CvData): boolean {
